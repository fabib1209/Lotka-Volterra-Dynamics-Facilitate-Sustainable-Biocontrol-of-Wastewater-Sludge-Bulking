# Lotka-Volterra Dynamics Facilitate Sustainable Biocontrol of Wastewater Sludge Bulking

This repository is a collection of scripts that guide you through the data analyses of the paper of Lotka-Volterra Dynamics Facilitate Sustainable Biocontrol of Wastewater Sludge Bulking ( doi: https://doi.org/10.1093/ismejo/wraf235). The raw data are available under data in this repository. Intermediate files, as well as results and figures, can be obtained by running the data analysis scripts.

